...
#### What should our VDP's scope section look like when all internet-accessible systems and services are covered?
Some are concerned that their VDP's scope will be used as a "roadmap for malicious actors". Like we say in the [FAQ about internet-accessible systems](#internet-accessible), the intent isn't for your VDP to be "a public inventory" of all your services on the internet, but simply to relay that your internet-accessible systems are in scope. Action 4 of the directive ([and its footnote](#fnref:15)) gives some perspective on how to handle this: implicitly add services by using a wildcard on the domain the host is on. When that doesn't work (perhaps because your agency doesn't manage the whole of the domain or it's only accessible via an IP address), consider representing in your policy that all internet-accessible systems and services are in scope, naming wildcarded second-level domains and key systems, and including the fact a system is in scope in that system's [system use notification](#should-we-make-changes-to-a-system-use-notification-when-it-is-in-scope-of-our-vdp).
