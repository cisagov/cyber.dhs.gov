...
_For an overview of this directive's requirements, review the [checklist](#checklist)._
...
* [Why is `p=reject` required?](#why-is-p-reject-required)
* [Does using `p=reject` mean that leads or anonymous sources may not be able to send us email?](#does-using-p-reject-mean-that-leads-or-anonymous-sources-may-not-be-able-to-send-us-email)
...
Having legitimate email inadvertently filtered is a common concern for any organization employing traditional spam-filtering technologies. DMARC is fundamentally different from spam-filtering in how it protects from malicious email, however.
