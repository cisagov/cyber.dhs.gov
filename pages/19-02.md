...
* [Is shared infrastructure (e.g., FedRAMP) in scope?](#is-shared-infrastructure-e-g--fedramp-in-scope)
* [How was my agency's scope put together?](#how-was-my-agencys-scope-put-together)
...
* [I fixed a vulnerability listed in my report. Can you rescan to verify?](i-fixed-a-vulnerability-listed-in-my-report-can-you-rescan-to-verify)
* [What’s the process for resolving false positives?](#whatss-the-process-for-resolving-false-positives)
...
The Directive applies to internet-accessible federal information systems, which encompasses those systems directly managed by an agency as well as those operated on an agency’s behalf.

By “internet-accessible,” we mean any system that is reachable over the public internet: it has a publicly routed IP address or a hostname that resolves publicly in DNS to such an address. It doesn’t pertain to infrastructure that is internal to your network that enables endpoints to be accessible over the internet, systems reachable from the internet but that require special configuration or access controls (e.g. via a Virtual Private Network), or shared services used by your agency that are not specifically managed by your agency or on behalf of your agency alone.

For example, if an agency has a critical vulnerability inside its network, BOD 19-02 doesn’t apply there. Local policy does, however, and we’d hope that policy aligns with the expectations for internet-accessible systems.
...
BOD 19-02 doesn’t apply to third-party infrastructure, considered alone. It does apply to *federal information systems*, that use, e.g., cloud service provider infrastructure.

For example, where an agency uses an infrastructure-as-a-service offering _and_ has a static, public IP address for the service it’s managing, that address should be shared with CISA for Cyber Hygiene scans. As of June 2019, Cyber Hygiene's scanning is limited to IPv4 addresses only. While other "as-a-service" cloud computing services are implicated by the directive, they should be accounted for in agency or vendor scans (systems using FedRAMP are scanned by the service provider).
...
#### How was my agency's scope put together?

Each agency’s scope of static, public IPv4 addresses was provided to CISA by the agency themselves when they began using the Cyber Hygiene service.

Per this directive, Federal agencies are required to notify CISA of changes to their [scope](LINK).

#### What types of changes to my agency’s scope do I need to report?

As your inventory of internet-accessible information systems change, your agency's designated technical POC – which is listed in your weekly Cyber Hygiene report – must notify <ncats@hq.dhs.gov>. You must also notify us when public, static IPv4 addresses are newly acquired, assigned to or operated by your agency (or operated on behalf of your agency), released or un-assigned.

#### The directive requires us to ensure Cyber Hygiene scans are not blocked. What does that mean?

You should not grant any preferential treatment for Cyber Hygiene scanning by e.g., explicitly whitelisting, blacklisting, or opening any ports/services other than what is normally available for your systems.

The idea is that CISA performs discovery from a perspective similar to that of an attacker. This perspective enables us to help gauge exposure and inform you on the urgency of response needed to threats and vulnerabilities. However, because our scanning is focused on identifying exposed vulnerabilities, and in an effort to detect them quickly, we make no attempt at stealth. This means our scans may sometimes trigger alerts that an attacker using more conservative tactics might not.
...
Should any automatic blocking of our scanning addresses occur, you must remove the block and notify <ncats@hq.dhs.gov> which addresses we were unable to connect to. This lets us jumpstart scanning on the particular IPs again.

#### Where can I locate Cyber Hygiene’s source IP addresses so my agency doesn’t add them to block lists?

The page containing Cyber Hygiene’s source IP addresses is included within the Methodology section of your weekly Cyber Hygiene reports. It can also be provided to you upon request to <ncats@hq.dhs.gov>.
...
To facilitate tracking, the ‘detection date’ of each vulnerability is included in the `findings.csv` attachment under appendix G in your Cyber Hygiene reports.

#### Will CISA alert closer to the point of initial detection?

Yes, as of June 2019. Within 24 hours of initial detection, an email will share summarized information on critical and high severity vulnerabilities when they are detected for the first time.
...
#### I fixed a vulnerability listed in my report. Can you rescan to verify?

No. Cyber Hygiene scanning is persistent and will automatically rescan for previously detected vulnerabilities. The frequency of scanning is based on each vulnerability’s severity level as described within the Methodology section of your Cyber Hygiene reports. If the Cyber Hygiene scans no longer detect the vulnerability, it will be listed in Appendix B.1: *Mitigated Vulnerabilities* of the weekly Cyber Hygiene report.

#### What’s the process for resolving false positives?

To report a Cyber Hygiene-identified vulnerability as a false positive, please submit an email through your agency’s POC to <ncats@hq.dhs.gov> (and cc: <CyberDirectives@hq.dhs.gov>) with analysis and supporting evidence for determination (for example, a screenshot of the IP address and operating system). CISA will review your submission and perform necessary analysis to validate the assertion. This will not include exploiting a vulnerability, but may include  sending packets to the host in question. CISA reserves the right to assert that findings are not false positives, but if CISA’s analysis appears to confirm your agency’s assertion, the vulnerability will be marked as a false positive.
